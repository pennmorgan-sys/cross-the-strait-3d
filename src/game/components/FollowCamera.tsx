...
import { isBoosting, getSteer } from '../input'
import { damp, lerp } from '../systems/math'
import { TANKER_SCALE } from '../constants'
...
export default function FollowCamera() {
  ...
  useFrame((_, dt) => {
    ...
    camera.position.lerp(tmp, damp(steady ? 10 : 6.5, d))
    look.set(p.x * 0.38, p.y + 2.5, p.z - hullLen * 1.8 - lead * 0.25)
    camera.lookAt(look)

    roll.current = lerp(roll.current, -getSteer() * 0.035, damp(4.5, d))
